...
The current leaderboard contains provisional validation results. The public test tasks and PDFs are available in the [public dataset](https://huggingface.co/datasets/amitbcp/docinsights-2026-shared-task-data). Test submissions are not open yet. Organizers will announce activation after the private scoring key is installed and verified. Performance on the held-out test set will determine the final leaderboard.
